...
let SDPTools = {
    ...
    increaseSessionVersion: function (session) {
        if(session && session.origin){
            ...
            console.info("Session updated version is " + sessionVersion);
        }
    },
    ...
    setH264Resolution: function(session, index, height){
        ...
        console.info('set levelId of local sdp ' + levelIdc)
     ...
     * @param AS
     */
    setMediaBandwidth: function (session, index, AS) {
        if(!session || !AS){
            return
        }
        ...
        if(media){
            if(media.bandwidth && media.bandwidth.length){
                media.bandwidth['AS'] = bandwidth['AS']
                media.bandwidth['TIAS'] = bandwidth['TIAS']
            }else {
                media.bandwidth = []
        ...
        if(index !== undefined){
            ...
            if(media.rtcpFb && media.rtcpFb.length){
                for(let j = 0; j<media.rtcpFb.length; j++){
                    if(media.rtcpFb[j].type.indexOf('goog-remb') >= 0 || media.rtcpFb[j].type.indexOf('transport-cc') >= 0){
